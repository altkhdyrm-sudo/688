...
const STORAGE_KEY = "school_biology_nervous_system_v1";
const DB_NAME = "biology_drawings_db";
const STORE_NAME = "student_drawings";
...
function getFilteredQuestions() {
  const list = activeSection === "source" 
    ? window.BIOLOGY_CHAPTER_01.sourceQuestions 
    : window.BIOLOGY_CHAPTER_01.enrichmentQuestions;
...
function renderHomeScreen() {
  const sourceLen = window.BIOLOGY_CHAPTER_01.sourceQuestions.length;
  const enrichmentLen = window.BIOLOGY_CHAPTER_01.enrichmentQuestions.length;
  
  const sourceAttempted = window.BIOLOGY_CHAPTER_01.sourceQuestions.filter(isQuestionAttempted).length;
  const enrichmentAttempted = window.BIOLOGY_CHAPTER_01.enrichmentQuestions.filter(isQuestionAttempted).length;
...
function renderQuestionScreen() {
  ...
  if (question.questionType === "multi-part") {
    ...
    if (question.questionType === "fixed-underlined-true-false") {
      const underlineText = question.fixedUnderlinedText;
      if (underlineText) {
        questionDisplay = question.question.replace(
          `<u>${underlineText}</u>`, 
          `<span class="border-b-2 border-primary-purple/70 pb-0.5 px-1 mx-1 font-black inline-block text-primary-purple bg-primary-purple/5 rounded-md leading-none select-all">${underlineText}</span>`
        );
      }
    }
  ...
  if (isShown) {
    modelAnswerHtml = `
      <div class="bg-green-50/50 border border-green-200 rounded-2xl p-5 mt-6 flex flex-col gap-3">
        ...
        <p class="model-answer-text text-sm font-medium leading-relaxed text-main-text whitespace-pre-wrap">${question.modelAnswer}</p>
      </div>
    `;
    ...
    <div class="flex flex-col gap-6 max-w-3xl mx-auto w-full">
      ...
      <div class="bg-white border border-borders rounded-3xl p-6 md:p-8 shadow-sm flex flex-col gap-5 relative mt-2">
        ...
        <h2 class="question-text text-lg md:text-xl font-extrabold text-main-text leading-relaxed mt-2">
          ${question.questionType === "fill" || question.questionType === "true-false-correction" || question.questionType === "fixed-underlined-true-false" ? "أكمل أو أجب عن المطلوب أدناه:" : question.question}
        </h2>
